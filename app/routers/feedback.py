...
async def create_new_feedback(
    ...
    try:
        ...
        print(f"Raw request body: {body.decode()}")  # Отладка
        
        # Пробуем распарсить
        import json
        data = json.loads(body.decode())
        print(f"Parsed data: {data}")  # Отладка
        
        # Валидируем через схему
        feedback = FeedbackCreate(**data)
        print(f"Validated feedback: {feedback.dict()}")  # Отладка
        
    except Exception as e:
        print(f"Validation error: {e}")  # Отладка
